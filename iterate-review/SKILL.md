...
12. **Fold merged findings + append ONE HISTORICAL block to the pass log.** The editor is the sole writer: fold findings into the working code and `code_corrections` mechanically.
    ...
    **When the block is opened — and why it may be opened before folding finishes.** The ordinary case is one append after folding completes. But step 12's two-phase card persistence requires a durable write *mid-fold*, before that append would otherwise have happened — and a card written nowhere is exactly the interruption the pending write exists to survive. So: **open the block the moment anything needs durable recording.** Write the `## Pass <N>` header and the `**Scope:** … **Diff size:** … **Verdict:** … **Posture:** … **Lenses:** …` line first — *every field on that line is already known at that point*, since the worst-of verdict is fixed by the merge in step 11, before the first finding is folded — then fill the sections beneath it incrementally as folding proceeds. There is still exactly **one** block per pass: "append ONE block" constrains the block count, not the number of writes.

    **On resume, complete the open block; never append a second.** A session that reads the log and finds a `## Pass <N>` block whose pass number is the pass in progress is looking at its own partial work — it continues filling that block. The `(pending)` card inside it is the finer-grained signal for *where* folding stopped (re-present that card, then carry on); the block header itself is the signal for *which* block to write into. A duplicate `## Pass <N>` block is always a bug.
...
16. **On Continue** (manual or loop-auto): increment pass count, loop to step 9. The next pass's `=== PRIOR PASSES ===` block now includes this pass's HISTORICAL section.
    ...
    **On Converge or Abort:** write the state file at `~/.claude/skills/iterate-review/state/<scope-hash>.json` with these fields:
    ...
    `confirmed_accepted_risks` is empty when the review never proposed one. Converge is impossible (per step 12's accounting table) while any `AR-<n>` remains `proposed` or `reopened`, so every entry here is `confirmed` at Converge time by construction — `Abort` may still leave `proposed`/`reopened` items unresolved. **The array is current-state, not historical: it lists exactly those items whose `confirmed` state still holds at the moment the file is written.** An item confirmed earlier and then invalidated — returned to `proposed` by a posture-digest mismatch, or to `reopened` by a rejection — is **absent**, even though it "was confirmed" at some point during the review. The field name is the contract: these are confirmations a consumer may act on, and a resume that trusted an invalidated one would proceed on a human decision that no longer applies to the current posture text. The pass log remains the authority on anything left pending — and on the fact that an absent item was ever confirmed at all.
