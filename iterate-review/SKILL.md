...
12. **Fold merged findings + append ONE HISTORICAL block to the pass log.** The editor is the sole writer: fold findings into the working code and `code_corrections` mechanically.
    ...
    **The floor is 2–3 *as a target*, not a hard minimum — context-sensitive omission can reduce it, down to 1.** A card needs **at least one genuine alternative** to the recommendation — a recommendation with no alternative at all isn't presenting a decision, it's announcing one. When a design-shaped-fold escalation has no cheaper alternative *and* `accept the risk` is excluded (trust-boundary code), the card legitimately has exactly one alternative — that's the floor, not a contract violation. **If omission would leave *zero* alternatives** (every option this card type could offer is prohibited for this specific item), that's a signal the moment isn't actually a decision at all — there is only one legal continuation, so proceed with it directly and record why in the pass log, rather than presenting a card with nothing to compare the recommendation against.
