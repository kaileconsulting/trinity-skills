...
9. **Prepare the pass inputs — the runner selects lenses and composes.** Write two files for the runner — **inside the trusted boundary**: the skill's `state/inbox/` handoff area (conventional), or anywhere inside the invoking repo root. The runner refuses paths (symlinks resolved) outside repo root + `state/inbox/` — the rest of the state root is deliberately excluded so one repo's review can never read another repo's artifacts — and any `--log-path` (override *or* default, symlinks resolved) must live inside the repo root. All of this exists so the pre-approved allowlist rule can never be used to read arbitrary host files into a network-backed codex prompt:
