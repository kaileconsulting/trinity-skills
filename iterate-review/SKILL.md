...
9. **Select lenses + compose per-lens input.** Apply the selection rules in `~/.claude/skills/iterate-review/lenses/README.md` (Matching semantics) to the captured diff: `senior-dev` always runs; `security` / `qa` run when their `path_globs` / `content_regexes` / `non_trivial_without_tests` match. Ambiguity biases toward inclusion. Then, for each selected lens, compose its input by concatenating (in order):
