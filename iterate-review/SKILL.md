...
9. **Prepare the pass inputs — the runner selects lenses and composes.** Write two files for the runner — **inside the invoking repo root**, conventionally `.git/iterate-review/` (inside the boundary, invisible to git, uncommittable; in a non-git cwd, anywhere under the cwd). The runner refuses any readable path (symlinks resolved) outside the repo root — no shared handoff area exists, deliberately, so one repository's review can never read files staged for another's — and any `--log-path` (override *or* default, symlinks resolved) must likewise live inside the repo root. All of this exists so the pre-approved allowlist rule can never be used to read arbitrary host files into a network-backed codex prompt:
