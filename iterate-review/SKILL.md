...
11. **Read, validate, and merge the lens responses.**
    ...
    - **Validate `register_ref` on findings — three gates, in order, all must pass.** For each finding carrying a `register_ref`, look it up in `docs/risk-posture.md`'s `## Accepted risks` section. Unknown id, malformed id, or a missing register file → treat the finding as untagged (drop the ref; note in the HISTORICAL block, e.g. "register_ref RR-x not found, treated as untagged") and disposition it normally.
      ...
      3. **Trust-boundary gate**, if this pass's posture includes a `PF-shipbar` field: the same anti-criterion `accepted-risk` itself carries — a register-match must never apply to code `PF-shipbar` names as a trust boundary requiring full rigor regardless of posture. If the finding concerns trust-boundary code, the match is invalid regardless of fit. (No `PF-shipbar` field available for this pass — e.g. the register-only case — means this gate has nothing to exclude against; it doesn't block matching.)
      ...
      **Before trusting any `register-match` carried in a `PRIOR PASSES` block, re-run all three gates against the *current* pass's context — not just the digest.** The digest recheck alone catches an amended entry, but the other two gates have inputs that can change between passes too: the currently resolved posture could add the finding's location to `PF-shipbar` on this pass (e.g. via a design-card decision mid-review), and a rebase or changed PR base could make the entry absent at the newly-resolved base revision. Recompute the digest, reconfirm the entry still exists unchanged at the current base revision, and reapply the current `PF-shipbar` exclusion. Any gate failing — not just the digest mismatch — invalidates the match: the finding reverts to needing a fresh disposition this pass, exactly like a newly-filed finding, and blocks Converge (step 14) until it receives one.
...
12. **Fold merged findings + append ONE HISTORICAL block to the pass log.** The editor is the sole writer: fold findings into the working code and `code_corrections` mechanically. **Append to the pass log via the Edit/Write tools, never shell heredocs** — log writes go through file-edit permissions, keeping the review's Bash surface to the single pre-approved runner rule. If the pass log doesn't exist, create it with a `# Code Review — <scope-tag>` H1 header — **load-bearing**: the runner reads an existing log as prior-pass context only when its first line is exactly that header for this scope tag (the format is the read capability; any file lacking the exact scope-tagged header is refused, and adopting another review's matching tag/header also adopts that same-repo review's identity — see `read_prior_passes`). Then append a single pass section (each finding tagged with its originating lens id):
    ...
    **Scope:** <scope value> · **Diff size:** <N lines> · **Verdict:** <APPROVE/REVISE/BLOCK> (worst-of; note any FAILED lenses) · **Posture:** <used <source>|absent|malformed → <resolution>|both-present (plan wins; repo shadowed)|register-only (no PF- fields; N entries)> · **Lenses:** <senior-dev[, security][, qa]>
    ...
    1. **<title>** — <severity> · lens: <lensid(s)>: <description>
       → Editor: <incorporated|skipped|disputed> — <reasoning, written by the editor when folding>
    2. ...
