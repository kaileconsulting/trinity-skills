...
- [x] Lens selection has exactly one implementation, and all 14 selection fixtures pass against it. *Met in Phase 1 (`bin/selection_engine.py` promoted; `check-selection.py` now a thin goldens CLI over it). iterate-plan's trivial always-all selection is fixture-pinned separately in Phase 3.*
- [x] A converged run leaves no state directory behind (model-invoked `prune-state --scope` at the convergence step). *Met — fixture-pinned in Phase 2 and exercised live at three real convergences (Phase 2's own review, doc-bot dogfood, Phase 3's review: 21 entries pruned).*
- [x] The extended parity fixture passes with both skills' runners in place. *Met in Phase 3 — 37/37 prose rules + sha256 byte-identity on `runner_shared.py` and `prune-state`; drift failure modes proven in `test-checkers.py`.*
- [x] `tools/check-all.sh` green; README documents the allowlist snippet(s) + Python floor; CHANGELOG entry per repo convention. *Met as of Phase 3 (122 + 62 runner fixtures, both allowlist lines in tilde form, CHANGELOG carries per-phase Added/Hardened sections).*
- [x] No hardcoded user paths anywhere in `bin/`; runners work via symlink and via plain copy, on macOS and Linux — verified by the install/platform matrix: macOS = authoring machine, **including allowlist-rule matching demonstrated in both layouts** (symlink layout via the dogfood review; copy layout via a copied-directory install with its corresponding rule, one scripted review step run without prompts); Linux = `python:3.9-slim` container running `tools/check-all.sh` + runner fixtures in both layouts (Docker verified available on the authoring machine, server 29.6.2). Rule matching on Linux is not separately demonstrable in a container (no harness there) and is covered by the path-prefix argument stated in Who / Use cases. *Met 2026-08-11: Linux container ran the full check-all green on Python 3.9.25 (122+62 fixtures, parity, checker self-tests) plus a symlink-layout smoke — the run surfaced and fixed one checker environment assumption (the read-only-dir prune fixture can't refuse root; now an explicit euid-0 skip, with real coverage retained on the non-root authoring machine). macOS copy-layout demo: `cp -R` install + its absolute-path rule ran `prune-state` and `run-pass --help` with zero prompts; the copy resolved its own state root. Demo artifacts and rule removed after.*
