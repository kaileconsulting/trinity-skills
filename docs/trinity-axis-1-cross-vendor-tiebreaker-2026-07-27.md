...
Phase 0 is required to choose its model default from a benchmark over real disputed-HIGH findings. **That corpus now exists but is small — 3 real disputes as of 2026-08-11 (now 4, see the 2026-08-13 update below), below any plausible benchmark threshold (Q9)** — and the history of measuring it says something important about the trigger itself.
...
2. **2026-08-10, runner-scripts Phase 2 review pass 6** (senior-dev): final-rmdir TOCTOU "can remove a replacement directory" — window real, harm impossible (POSIX `ENOTEMPTY`; verified live). Pass log: `docs/archive/code-review-commit-13c9e0c.md`.
3. **2026-08-10, doc-bot write-path Phase 1 pass 1** (security): "publish endpoint has no auth" — the plan's accepted posture (Tailscale perimeter, DevOps-closed question), invisible in the diff. Pass log in the doc-bot repo. **Update 2026-08-13 (Risk Posture Phase 3 retrospective mining):** this exact dispute — same finding, same underlying Q3 posture — actually recurred **three times**, not once: first at write-path Phase 0 pass 1 (2026-08-06, one phase *before* this corpus entry's date), again here at Phase 1 pass 1, and a third time at Phase 3 pass 1 (2026-08-12, "approve/cancel expose irreversible forge actions without authentication," disputed with "the same reasoning Kyle ratified in Phase 1"). Each recurrence cost a fresh dispute-and-cite cycle because no phase review had memory of the prior ones. This is direct, concrete evidence for exactly the failure mode the Risk Posture & Proportionality register (`docs/risk-posture.md`, shipped Phase 1 2026-08-13) is designed to close: a register entry citing this posture once would let the *lens itself* recognize the pattern and skip filing it, rather than the editor re-disputing it from scratch three times across three separate phase reviews of the same product.
4. **2026-08-13, doc-bot `merged-at-rev-0007` pass 6** (senior-dev): "concurrent approve and cancel could record mutually contradictory outcomes" — a genuinely distinct dispute (different code, different evidence type: a design-tradeoff argument, "the forge is the source of truth, this database is a hint," not an external fact-check) from items 1–3 above. Disputed with the evidence the finding itself asked for (a reachability proof plus a test in both orders) and held at the next pass (the reviewer read the argument and did not re-file). Pass log in the doc-bot repo.

All four match the predicted causal pattern — the reviewer read *code with constraints not visible in the diff* — and none arose in the prose-review era. Each was arbitrated with **external evidence or an explicit design-tradeoff argument** (an interpreter run, POSIX semantics, plan history, or a reachability proof): the exact job this plan gives the tiebreaker. Observed rate so far: roughly one disputed HIGH per major multi-pass code review, plus — newly visible from the Phase 3 retrospective — the *same* dispute can recur multiple times across phases of one product absent a persistent-memory mechanism (item 3's history). Corpus: **4/6** as of 2026-08-13.
